...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="headerRow">
        ...
        <xsl:text>place/no placeTerm</xsl:text>
        <xsl:value-of select="$separator"/>
        <xsl:text>placeTerm</xsl:text>
    ...
    <xsl:template match="mods:mods">
        ...
        <!-- This script will select each parent element, show values in one column if no child is present, and show values in next column when child is present -->
